...
Kanchi runs wherever Docker runs. For production, point it at PostgreSQL or MySQL. For development, SQLite works great.
...
<Cards className="mt-6">
  <Card href="#docker-compose" title="Docker Compose" description="Production deployment modeled after Docmost — simple and reliable." />
...
<Callout>
Deploying on MySQL? Set `DATABASE_URL` to your MySQL DSN (for example: `mysql+aiomysql://user:pass@mysql:3306/kanchi`).
</Callout>
...
<Steps>
  <Step>
    **Create docker-compose.yaml**
    ...
    <Tabs items={['SQLite (Quick Start)', 'PostgreSQL (Production)', 'Bring Your Own DB']} className="border-gray-800 [&_[role=tab]:hover]:text-white">
      <Tab value="sqlite">
        ```yaml
        services:
          kanchi:
  ...
  <Step>
    **Access the dashboard**
    ...
    The dashboard footer and `/api/health` response include the running version to speed up support requests.
  </Step>
</Steps>
...
<Accordions type="single" collapsible>
  ...
  <Accordion title="Frontend can't connect to backend" value="cors">
    **Check API URLs**: Docker deployments use same-origin URLs by default. For local source development, verify environment variables:
