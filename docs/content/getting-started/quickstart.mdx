...
<Steps>
    ...
    services:
      kanchi:
        ...
        environment:
          ...
          # Optional: URL handling for proxies and path-prefixed installs
  ...
  <Step>
    **Start Kanchi**
    ...
    This pulls the latest image from Docker Hub and starts the container. Access the dashboard at [http://localhost:3000/ui](http://localhost:3000/ui).
  </Step>
...
<Steps>
  ...
  <Step>
    **Start the application**
    ...
    This starts the FastAPI backend on port `8765` and the Nuxt development frontend on port `3000`.
  </Step>
...
<Steps>
  <Step>
    **Check backend health**
    ...
    ```bash
    # Docker / production-style single process
...
<Accordions type="single" collapsible>
  ...
  <Accordion title="Frontend can't connect to backend" value="frontend-connection">
    **Check API URLs**: Docker deployments use same-origin URLs by default. For local source development, verify `NUXT_PUBLIC_API_URL` and `NUXT_PUBLIC_WS_URL` point to your backend.
