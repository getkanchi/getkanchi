...
- **TLS Support**: HTTPS and WSS
- **CORS Configuration**: Restrict origin access
- **Rate Limiting**: Prevent abuse

### Data Privacy

- **Sensitive Args**: Optionally hide task arguments
...
```
Kanchi → PostgreSQL → Celery Broker
```
...
Load Balancer
  ├─> Kanchi Instance 1 ─┐
  ├─> Kanchi Instance 2 ─┤
  └─> Kanchi Instance 3 ─┴─> PostgreSQL (Primary/Replica)
```
...
```
Region 1: Kanchi + PostgreSQL Replica (read)
Region 2: Kanchi + PostgreSQL Primary (write)
Region 3: Kanchi + PostgreSQL Replica (read)
```
